...
-- Authoritative Atomic Settlement Timestamp
-- ------------------------------------------------------------
-- PURPOSE
-- Replace the existing exactly-once settlement function so a
-- successful live settlement atomically:
--
-- 1. Claims one existing SUBMITTED execution
-- 2. Verifies Coinbase order identity
-- 3. Locks the client's pending asset bucket
-- 4. Consumes authoritative filled USD exactly once
-- 5. Marks the SAME execution SETTLED
-- 6. Records authoritative settled_at = now()
--
-- SAFETY
-- - Multi-Asset only
-- - No Coinbase calls
...
-- - Settlement status, pending consumption, and settled_at
...
begin
   ...
   * another reconciliation cannot consume the same fill again.
   */
  ...
  /*
   * Coinbase identity must exactly match the persisted
  ...
  /*
   * Never consume more pending capital than is currently
   * assigned to this asset.
   *
   * This intentionally fails closed instead of allowing a
   * negative or fabricated pending balance.
   */
   ...
   * ========================================================
   *
   * Status transition, authoritative settlement timestamp,
   * and pending-capital consumption occur inside the SAME
...
is
  'Atomically settles one previously submitted Atlas Multi-Asset Coinbase execution, consumes its authoritative filled USD from exactly one client pending allocation bucket, and records the authoritative settlement timestamp. Prevents duplicate fill settlement by locking the execution row and transitioning SUBMITTED to SETTLED in the same transaction.';