...
// - Pulse credentials live in public.coinbase_keys
// - Atlas credentials live in public.atlas_coinbase_keys
//
// NOTE:
...
async function fetchBtcSpotUsd(): Promise<number> {
  ...
  if (!r.ok) {
    throw new Error(`btc_spot_http_${r.status}`);
  }
  ...
  if (!Number.isFinite(p) || p <= 0) {
    throw new Error("btc_spot_parse_failed");
  }
